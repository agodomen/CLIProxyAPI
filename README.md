...
flowchart TD
    ...
    U_dev -->|"① 同步社区 dev 分支"| L_dev
    L_dev -->|"② 拉取功能分支"| L_feat
    L_feat -->|"③ 提交 PR 到社区"| U_dev
    U_dev -->|"④ 社区合并 PR"| U_main

    U_main -->|"⑤ 同步到本仓库 main"| L_main
    L_main -->|"⑥ rebase 到 debug"| L_debug

    L_feat -.->|"⑤' 社区未接受 PR<br/>直接合并到 debug"| L_debug

    L_debug -->|"⑦ 合并到 release"| L_release

    style upstream fill:#e1f5fe,stroke:#01579b
    style local fill:#fff3e0,stroke:#e65100
    style U_dev fill:#b3e5fc,stroke:#0277bd
    style U_main fill:#b3e5fc,stroke:#0277bd
    style L_dev fill:#c8e6c9,stroke:#2e7d32
    style L_feat fill:#fff9c4,stroke:#f9a825
    style L_main fill:#ffe0b2,stroke:#ef6c00
    style L_debug fill:#d1c4e9,stroke:#512da8
    style L_release fill:#f8bbd9,stroke:#c2185b
```
